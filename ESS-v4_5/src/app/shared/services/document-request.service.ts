...
import { DocumentRequest } from '../interfaces/interfaces';
...
export class DocumentRequestService {
  ...
  create(shortcut: DocumentRequest): Observable<DocumentRequest> {
    return this.cabietStructureConfig$.pipe(
      take(1),
      switchMap((cabietStructureConfig) =>
        this.http
          .post<DocumentRequest>('/DMSDocumentRequest/create-document-request', { shortcut })
          .pipe(
  ...
  update(shortcut: DocumentRequest): Observable<DocumentRequest> {
    const payload = {
      id: shortcut.id,
  ...
  delete(id: string): Observable<boolean> {
    return this.http
      .delete<boolean>(`${environment.baseUrl}/DMSDocumentRequest/delete-document-request`, {
        params: { id },
      })
      .pipe(
        tap(() => {
