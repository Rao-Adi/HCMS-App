...
export class RoleService {
  ...
  create(shortcut: Role): Observable<Role> {
    return this.cabietStructureConfig$.pipe(
      take(1),
      switchMap((cabietStructureConfig) =>
        this.http.post<Role>('/DMSRole/create-role', { shortcut }).pipe(
  ...
  update(shortcut: Role): Observable<Role> {
    const payload = {
      id: shortcut.id,
    ...
    return this.http
      .put<Role>(`${environment.baseUrl}/DMSRole/update-role`, payload, {
        headers,
      })
      .pipe(
        tap((updated) => {
  ...
  delete(id: number): Observable<boolean> {
    return this.http
      .delete<boolean>(`${environment.baseUrl}/DMSRole/delete-role`, {
        params: { id },
      })
      .pipe(
        tap(() => {
