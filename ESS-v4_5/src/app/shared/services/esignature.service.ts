...
import { ESignature } from '../interfaces/interfaces';
...
export class ESignatureService {
  ...
  create(shortcut: ESignature): Observable<ESignature> {
    return this.cabietStructureConfig$.pipe(
      take(1),
      switchMap((cabietStructureConfig) =>
        this.http.post<ESignature>('/DMSESignature/create-esignatures', { shortcut }).pipe(
  ...
  update(shortcut: ESignature): Observable<ESignature> {
    const payload = {
      id: shortcut.id,
    ...
    return this.http
      .put<ESignature>(`${environment.baseUrl}/DMSESignature/update-esignatures`, payload, {
        headers,
      })
      .pipe(
        tap((updated) => {
  ...
  delete(id: string): Observable<boolean> {
    return this.http
      .delete<boolean>(`${environment.baseUrl}/DMSESignature/delete-esignatures`, {
        params: { id },
      })
      .pipe(
        tap(() => {
