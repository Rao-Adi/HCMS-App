...
export class ViewDocumentPendingApproval {
  ...
  openDocumentModal(rowData: any) {
    ...
    // The Document Type's template decides what the reader gets, not whether a file happens to
    // be attached. A Word template -- which is what Policy and SOP use -- is merged with this
    // content and downloaded, so the document is read as it will actually be issued: header,
    // document number, version, signature block and all.
    //
    ...
    // download -- the behaviour this screen had for the file case already.
